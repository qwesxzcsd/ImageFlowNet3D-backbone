    ...
    if segmentor:
        ...
        if run_count is None:
            ...
            if len(existing_runs) > 0:
                #run_counts = [int(item.split('/')[-2].split('run_')[1]) for item in existing_runs] #forward slash on windows doesn't work
                run_counts = []
                for path in existing_runs:
                    norm = os.path.normpath(path)              # normalize to handle both “\” and “/”
                    folder = os.path.basename(norm)
                    if not folder.startswith("run_"):
                        continue
                    try:
                        run_counts.append(int(folder.split("run_")[1]))
                    except ValueError:
                        continue
