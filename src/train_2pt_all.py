...
def train(config: AttributeHashmap):
    ...
    print(f"[DEBUG] SyntheticDataset.num_image_channel = {num_image_channel}")
    print(f"[DEBUG] SyntheticDataset.max_t = {max_t}")
...
def train_epoch(config: AttributeHashmap,
    ...
    print("→ Saving outputs to:", config.save_folder)
    ...
    for iter_idx, (images, timestamps) in enumerate(tqdm(train_set)):
        ...
        if iter_idx % config.batch_size == config.batch_size - 1:
            optimizer.step()
            scheduler.step() #pytorch 1.1.0 debug recommendation moving scheduler.step before optimizer
            optimizer.zero_grad()
            ema.update()
...
def plot_contour(image, label):
    ...
    true_contours, _hierarchy = cv2.findContours(lbl,
                                                 cv2.RETR_TREE,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu-id', help='Index of GPU device', type=int, default=0)
